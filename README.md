...
Stats is a lightweight dashboard for your Mac's system health and Amp usage. Use it as a native menu bar app or run it directly in a terminal.
...
- Amp token activity, model usage, and source usage
- Four customizable world clocks

Stats reads usage exclusively through the installed Amp CLI. It does not read or store Amp credentials.

## Install

Stats requires macOS 11 or newer and currently provides prebuilt releases for Apple Silicon Macs. [Amp](https://ampcode.com/) must also be installed and signed in.
...
Run `stats --help` to see all CLI options. Enabled Amp sections require the Amp CLI to be available in `PATH`.
